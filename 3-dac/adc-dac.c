...
int main(void)
{
    ...
    while (1)
    {
        ...
        adc = adc_reading * 0.00080566406; // 3.3*10^-12
        dac = dac_setting * 0.00080566406;
        diff = adc - dac;

        printf("ADC: %.4fV, DAC: %.4fV, ADC-DAC: %.4fV", adc, dac, diff);
        printf("\r\n");

        dac_setting = dac_setting + increment;

        if (dac_setting >= 4095 || dac_setting == 0)
        {
            increment = -increment;
        }
        /* USER CODE END WHILE */
